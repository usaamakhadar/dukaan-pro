      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 sm:p-12 lg:p-24 overflow-y-auto bg-[#f9f9fb] lg:bg-white relative">
        ...
        <div className="w-full max-w-md">
          ...
          <div className="text-center lg:text-left mb-10 mt-8 lg:mt-0">
             <h2 className="text-3xl sm:text-4xl font-extrabold text-[#141b2d] mb-3 tracking-tight">{t('register_header')}</h2>
             <p className="text-zinc-500 font-medium tracking-wide">{t('register_subheader')}</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-6">
            ...
            >
              {loading ? t('register_loading') : (
                <>
                  {t('register_button')} <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />
                </>
              )}
            </Button>
          </form>
          ...
          <p className="text-center mt-12 text-sm text-zinc-500 font-semibold">
            {t('have_account')} <Link href="/login" className="font-extrabold text-blue-600 hover:text-blue-700 transition-colors underline underline-offset-4 decoration-2 decoration-blue-200 hover:decoration-blue-600">{t('login_link')}</Link>
          </p>
        </div>
      </div>
    </div>
